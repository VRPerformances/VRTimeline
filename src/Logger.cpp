...
void Logger::setup(){
	gui2 = new ofxUICanvas(ofGetWidth() - width, 0, width, 25);
	gui = new ofxUIScrollableCanvas(ofGetWidth() - width, 25.0f, width, height);    
...
void Logger::handleResize(){
	gui->setPosition(ofGetWidth() - width, 25.0f);
	gui2->setPosition(ofGetWidth() - width, 0.0f);
}